...
import { concatenateNFAs } from './lib/concatenation';
import AutomatonGraph from './components/AutomatonGraph';

const DEFAULT_NFA1: NFA = {
  states: ['q0', 'q1'],
  alphabet: ['a', 'b'],
  transitions: {
    'q0': { 'a': ['q1'], 'b': ['q0'] },
...
export default function App() {
  ...
  const [nfa1, setNfa1] = useState<NFA>(DEFAULT_NFA1);
  const [nfa2, setNfa2] = useState<NFA>(DEFAULT_NFA2);
  const [editingNfa, setEditingNfa] = useState<1 | 2>(1);
  const nfa = editingNfa === 1 ? nfa1 : nfa2;
  const setNfa = editingNfa === 1 ? setNfa1 : setNfa2;
  ...
  const startConstruction = () => {
    // Initial merge: just states and original transitions
  ...
  const stepToLinking = () => {
    ...
    // Check if start states of NFA1 hit final states
    const startHitsFinal = nfa1.startStates.some(s => nfa1.finalStates.includes(s));
    if (startHitsFinal) {
      setResultNfa(prev => prev ? ({
        ...prev,
        startStates: Array.from(new Set([...prev.startStates, ...nfa2.startStates]))
      }) : null);
    }
  ...
  const stepLinking = () => {
    ...
    // Find all (q, a) in NFA1 that hit a final state
    const links: { q: StateId, a: Symbol }[] = [];
    for (const q of nfa1.states) {
      for (const a of nfa1.alphabet) {
        const targets = nfa1.transitions[q]?.[a] || [];
        if (targets.some(t => nfa1.finalStates.includes(t))) {
          links.push({ q, a });
        }
      }
    }
    ...
    if (currentLinkIndex < links.length) {
      ...
      setResultNfa(prev => {
        ...
        const q_new = q;
        newTransitions[q_new] = { ...newTransitions[q_new] };
        newTransitions[q_new][a] = Array.from(new Set([...newTransitions[q_new][a], ...nfa2.startStates]));
  ...
  const runToCompletion = () => {
    const result = concatenateNFAs(nfa1, nfa2);
    setResultNfa(result);
    setConstructionPhase('finished');
  };
  ...
  const reasonTransitionsNfa1 = useMemo(() => {
    ...
    if (currentLinkIndex > 0 && currentLinkIndex <= links.length) {
      ...
      const targets = nfa1.transitions[q]?.[a] || [];
      return targets
        .filter(t => nfa1.finalStates.includes(t))
        .map(t => ({ from: q, to: t, label: a, color: "#fbbc33" }));
    }
    return [];
  }, [constructionPhase, currentLinkIndex, nfa1]);
  ...
  const resultHighlightedTransitions = useMemo(() => {
    ...
    if (currentLinkIndex > 0 && currentLinkIndex <= links.length) {
      ...
      // Add the "reason" transitions (prefixed for resultNfa)
      const targets = nfa1.transitions[q]?.[a] || [];
      targets
        .filter(t => nfa1.finalStates.includes(t))
        .forEach(t => {
      ...
      nfa2.startStates.forEach(s => {
        highlights.push({ from: q, to: s, label: a, color: "#e4003a" });
      });
    }
    return highlights;
  }, [constructionPhase, resultNfa, currentLinkIndex, nfa1, nfa2]);

  // NFA Editor helpers
  const addState = () => {
    const prefix = editingNfa === 1 ? 'q' : 'p';
          ...
          {activeTab === 'edit' && (
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              <div className="flex gap-2">
                <button 
                  onClick={() => setEditingNfa(1)}
                  className={`flex-1 py-2 text-[10px] font-bold uppercase border border-th-black transition-colors ${editingNfa === 1 ? 'bg-th-dunkelblau text-th-white border-th-dunkelblau' : 'hover:bg-th-sand/10'}`}
                >
                  {t('nfa1')}
                </button>
                <button 
                  onClick={() => setEditingNfa(2)}
                  className={`flex-1 py-2 text-[10px] font-bold uppercase border border-th-black transition-colors ${editingNfa === 2 ? 'bg-th-dunkelblau text-th-white border-th-dunkelblau' : 'hover:bg-th-sand/10'}`}
                >
                  {t('nfa2')}
                </button>
              </div>
                    ...
                    <div className="text-xs space-y-2">
                      ...
                      {constructionPhase === 'linking' && (
                        <>
                          ...
                          {nfa1.startStates.some(s => nfa1.finalStates.includes(s)) && (
          ...
          <div className="bg-th-white p-6 border border-th-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] min-h-[700px] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center gap-4 text-[10px] font-bold uppercase opacity-50">
                ...
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[#bda2c1] border border-th-black"></div> {t('nfa2')}</div>
            ...
            <div className="flex-1 flex flex-col gap-6 min-w-0 overflow-hidden">
              ...
              <div className="flex-1 flex flex-col min-w-0 space-y-2">
                <h3 className="text-[10px] uppercase font-bold opacity-50 text-th-dunkelblau">{t('nfa2_input')}</h3>
                <div className="flex-1 min-h-0 min-w-0 bg-th-white border border-th-sand/20 rounded">
                  <AutomatonGraph 
                    states={nfa2.states}
                    transitions={Object.entries(nfa2.transitions).flatMap(([from, syms]) => 
                      Object.entries(syms).flatMap(([sym, tos]) => 
                        tos.map(to => ({ from, to, label: sym }))
                      )
                    )}
                    startStates={nfa2.startStates}
                ...
                <div className="flex-1 flex flex-col min-w-0 space-y-2">
                  ...
                  <div className="flex-1 min-h-0 min-w-0 bg-th-white border border-th-sand/20 rounded">
                    <AutomatonGraph 
                      ...
                      stateColor={(id) => nfa1.states.includes(id) ? "#719cbf" : "#bda2c1"}
                    />
                  </div>
                </div>
              )}
